...
if df is None:
    try: df = pd.read_csv("BlackFriday_Cleaned.csv")
    except FileNotFoundError: pass

if df is None:
    st.error("Dataset not found. Please place BlackFriday_Cleaned.zip in the app directory.", icon="🚨")
    st.stop()
...
avg_basket = df.groupby(“User_ID”)[“Purchase”].sum().mean() if total_tx else 0
...
if df.empty:
st.warning(“No data matches the current filters.”, icon=“⚠️”); st.stop()
...
top_city_pct = df.groupby(“City_Category”)[“Purchase”].sum().max() / total_rev * 100
...
anom_pct  = n_anomaly / total_tx * 100
...
# Find 80% threshold
thresh_idx = (user_rev["CumPct"] >= 80).idxmax()
thresh_x   = user_rev.loc[thresh_idx,"UserPct"]
...
missing_pc2 = raw_df["Product_Category_2"].isna().sum()
missing_pc3 = raw_df["Product_Category_3"].isna().sum()
...
seg_legend = "".join([
    f'<span class="cpill"><span class="cdot" style="background:{SEG_CLR[s]}"></span>{s}</span>'
    for s in SEGS
])
...
m4.metric("Anomalies",    f"{len(anom_df):,}  ({len(anom_df)/len(dfa)*100:.2f}%)")
...
top_a = (anom_df.sort_values("Purchase",ascending=False)
                .head(20)[["User_ID","Product_ID","Gender_Label","Age_Label",
                           "City_Category","Occupation","Purchase","Z_Score"]]
                .copy())
top_a["Purchase"] = top_a["Purchase"].map(lambda x:f"₹{x:,}")
top_a["Z_Score"]  = top_a["Z_Score"].map(lambda x:f"{x:.2f}")
top_a = top_a.rename(columns={"Gender_Label":"Gender","Age_Label":"Age",
